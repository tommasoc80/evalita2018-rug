...
#    x, y, vocabulary, vocabulary_inv_list, len_train, idx_espresso = data_helpers_final_run.load_data() # idx_espresso is irrelevant in this case
    x, y, vocabulary, vocabulary_inv_list, len_train,test_ids = data_helpers_final_run.load_data() # idx_espresso is irrelevant in this case

    vocabulary_inv = {key: value for key, value in enumerate(vocabulary_inv_list)}
    y = y.argmax(axis=1)

    return x, y, len_train, vocabulary_inv,test_ids

# Get training data
print("Load data...")
X, Y, len_train, vocabulary_inv,test_ids = load_data(data_source)
# print("Xtrain shape:", Xtrain.shape)
print("Lenght train: {:d}".format(len_train))
...
#save_to = open('TEST-FB-FB-cnn.p', 'wb')
#save_to = open('TEST-TW-TW-cnn.p', 'wb')
#save_to = open('TEST-FB-TW-cnn.p', 'wb')
save_to = open('TEST-TW-FB-cnn.p', 'wb')
