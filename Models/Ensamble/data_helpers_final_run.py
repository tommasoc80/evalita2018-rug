...
def load_data_and_labels():
    ...
    samples, labels = [],[]
#    with open('../../Data/haspeede_FB-train.tsv','r', encoding='utf-8') as fi: # FB data
    with open('../../Data/haspeede_TW-train.tsv', 'r', encoding='utf-8') as fi:  # TW data
    ...
    Xtest = []
    test_ids = []
    with open('../../Data/Test/haspeede_FB-test.tsv','r', encoding='utf-8') as fi: # FB data
#    with open('../../Data/Test/haspeede_TW-test.tsv', 'r', encoding='utf-8') as fi:  # TW data
        for line in fi:
            data = line.strip().split('\t')
            # get sample
            Xtest.append(data[1])
...
#    return [Xtrain, Ytrain, Xtest, len_train, idx_espresso]
    return [Xtrain, Ytrain, Xtest, len_train,test_ids]
...
#    Xtrain, Ytrain, Xtest, len_train, idx_espresso = load_data_and_labels()
    Xtrain, Ytrain, Xtest, len_train, test_ids = load_data_and_labels()
...
#    return [X, Y, vocabulary, vocabulary_inv, len_train, idx_espresso]
    return [X, Y, vocabulary, vocabulary_inv, len_train, test_ids]
