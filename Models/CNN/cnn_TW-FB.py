...
def load_data(data_source):
    ...
    x, y, vocabulary, vocabulary_inv_list, len_train, test_sentence, test_ids = data_helpers_TW_FB.load_data()
    vocabulary_inv = {key: value for key, value in enumerate(vocabulary_inv_list)}
    y = y.argmax(axis=1)

    return x, y, len_train, vocabulary_inv, test_sentence, test_ids
...
# Get full dataset
print("Load data...")
X, Y, len_train, vocabulary_inv, test_text, test_ids = load_data(data_source)
...
fname = 'evalita_rug_TW-FB_cnn.txt'

with open(outdir + '/' + fname, 'w', encoding='utf-8') as fo:
    assert len(Yguess) == len(test_text), 'Unequal length between samples and predictions!'
    for idx in range(len(Yguess)):
        print(test_ids[idx] + "\t" + test_text[idx] + '\t' + Yguess[idx], file=fo) # binary task (coarse)
